...
export function GoldenLayoutLines(props: LayoutLinesProps) {
  ...
  return (
    <div className={cn("hidden lg:block layout-lines-1 pointer-events-none absolute h-full w-full overflow-hidden", className)} {...rest}>
      ...
      {!isBlogPost && ( <Line o="bottom" className={cn("right-0 top-[calc(61.8vh-57px)] w-[38.2%] scale-x-0", isBlog && "hidden 2xl:block")}  ref={line2} />  )}
        {!isBlog && !isBlogPost && (
            <Line o="right" className="hidden left-[76.39999999999%] top-[calc(61.8vh-57px)] lg:block scale-y-0" ref={line3} />
        )}
    </div>
  );
}
